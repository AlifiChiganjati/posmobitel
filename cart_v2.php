...
if($_SESSION['ROLE'] == "CUSTOMER")	{
	...
	if($merk == "poco"){
		$queryxx = mysql_query("SELECT id_outlet_rai FROM customer_mobitel WHERE nohp = '$user'");
	...
	//$query = mysql_query("SELECT id_outlet_rti, nama, id_outlet_rai, unit_bisnis FROM customer_mobitel WHERE (nama LIKE '%$input%' or nohp LIKE '%$input%') and (idsales_rti ='$idsales' or idsales_cesa='$idsales') limit 5");
		$queryxx = mysql_query("SELECT id_outlet_rti FROM customer_mobitel WHERE nohp = '$user'");
	}
	$datc = mysql_fetch_row($queryxx);
	$idcustom = $datc[0];
	mysql_close();
}
	...
	if((strlen($bank) > 0) && (strlen($idcustomer) > 0)){
		...
		if($total_belanja > 0){
			...
			$nofaktur = $ub."-".$date."-".rand(10, 99);
				...
				$prefix = "";
				$arrid = data_salesman($idsales);
				if($arrid){
					$prefix=$arrid->cluster;
				}
				$prefix = (strlen($prefix) > 1)? $prefix : "XX";
				
				$xx = "select * from order_temp where user='$user' and kategori like '$cat%'";
				$qryx = mysql_query($xx);
				$no_faktur = $ub."-".$prefix."-".$date."-".str_shuffle($date2);
				while($datax=mysql_fetch_object($qryx)){
					...
					if(mysql_query($qryz)){
						mysql_query("delete from order_temp where id_produk='$datax->id_produk' and user='$user'");
						$sts = true;
					}else{
						$sts = false;
						$msg .= "error $datax->id;";
	...
	if($jums == 0){
		...
		if($_SESSION['ROLE'] == "CUSTOMER")	{
			echo "<div class='input-box'>
						<input type='text' class='form-control mt-1' required='' placeholder='ID Pelanggan' id='idcustomer' name='idcustomer' readonly value='$idcustom'></input>	
						<i class='iconly-Profile icli'></i>
					</div>
					<div class='input-box'>
						<input type='text' class='form-control mt-1' placeholder='Nama Pelanggan' id='namacustomer' name='namacustomer' autocomplete='Off' readonly value='".$_SESSION['NAME']."'></input>	
			...
			echo "<footer class='footer-wrap shop bg-dark'>
				  <ul class='footer'>
