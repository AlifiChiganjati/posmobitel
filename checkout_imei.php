	...
	if($idsales && $idcustomer && $qty_e && $product_e && $via){
			$arrsales = data_salesman($idsales);
				if($arrsales){
					...
					//Pengkondisian Jika ada CTA TABANAN 2 MAKA UBAH JADI CTA TABANAN (NANTI BISA DI COMMENT APABILA ADA PENAMBAHAN DEPARTEMEN)
					if($data1 == "CTA TABANAN2"){
						$data1 = "CTA TABANAN";
					}
					...
					if($row > 0){
						while($sx = mysql_fetch_object($sql)){
							//mysql_query("update sales_invoice set id_customer='$idcustomer', via='$via', status = 1, no_faktur='$nofaktur', taxdate='$taxdate', transdate='$transdate', duedate= '$tempo', data1='$data1', data2='$data2' where id='$sx->id'");
							...
							//update status di imei gudang = 1
							mysql_query("update imei_gudang set status = 1 where imei='$sx->description'");
							$imtrack = "insert into imei_track (date_track, imei, idsales, wh_from, send_to, reff, status) values (now(), '$sx->description', '$idsales', '$sx->warehouse_name', '$idcustomer', '$nofaktur', 'OUT')";
							mysql_query($imtrack);
						}
								...
								<div class='offcanvas-body small'>
									<div class='app-info'>
										...
										<div class='content'>
											...
											Penjualan SP Fisik berhasil dilakukan!
											<a href='print_struk_temp.php?det=$enfisik' class='btn-solid-se'>PRINT</a>
										</div>
									</div>
								</div>
							</div>";
						...
						echo "<script language=javascript> location.href = 'print_struk_fisik_temp.php?det=$enfisik'; </script>";
...
$sql = "select sum(qty) as jml, sum(total) as total, id_product, id_customer from sales_invoice where idsalesman = '$idsales' and status = 0 and tipe='SN1' group by id_product";
$msq = mysql_query($sql);
//echo $sql;
