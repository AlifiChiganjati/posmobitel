	...
	if($idsales && $imei && $idcustomer){
		...
		if($product){
				if(is_queueing($imei)){
					...
					//$iscustomer = is_exist_customer($idcustomer);
							...
							if($range > 0) {
								if($range <= 100){
									...
									for ($i=0;$i<$range;$i++) {
										...
										$is_product_ksy = cek_produk_ksy($no_imei, $gudang);
										if($is_product_ksy){
												$res = 2;
										}else if(is_queueing($no_imei)){
											$res = 0;
											...
											if($res){
													...
													if($product){
														...
														$sql = "insert into sales_invoice (date_order, idsalesman, id_product, qty, unit_price, total, warehouse_name, id_customer,
														 detail_notes, description, opr, tipe, data2) values (now(), '$idsales', '$product', 1, $harga, $harga, '$gudang', '$idcustomer', '$no_imei', '$no_imei', '$idsales', 'SN1','Kantor Pusat')";
													...
													<div class='offcanvas-body small'>
														<div class='app-info'>
															...
															<div class='content'>
																...
																<a href='#'>$product terdeteksi produk KONSINYASI, silahkan hubungi admin</a>
															</div>
														</div>
													</div>
												</div>";
								...
								if($is_stok_exist){
									$sql = "insert into sales_invoice (date_order, idsalesman, id_product, qty, unit_price, total, warehouse_name,
									 id_customer,
									 detail_notes, description, opr, tipe,data2) values (now(), '$idsales', '$product', 1, $harga, $harga, '$gudang', '$idcustomer', '$imei', '$imei', '$idsales', 'SN1','Kantor Pusat')";
									if($is_product_ksy){
										$msg = "<div class='offcanvas offcanvas-bottom addtohome-popup show' tabindex='-1' id='offcanvas'>
												<div class='offcanvas-body small'>
													<div class='app-info'>
...
$jumdt = mysql_num_rows($qsql);
	$rex = false; $newharga = "";
	$next = "<a class='btn-solid bg-dark' href='checkout_imei.php'>PROSES KE PEMBAYARAN</a>";
	$batal = "";
		...
		if($rs->is_request == 1){
			...
			$listdata .= "<div class='item-wrap'>
							<div class='media'>
								...
								<div class=''>
								  <h4 class='title-color font-xs mb-0'><small>$rs->description</small></h4>
								  <span class='content-color font-xs'><small>$rs->id_product </small></span>
								</div>
								$harga
							</div>
						</div>";
